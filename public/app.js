...
async function refreshBackendState() {
  ...
  if (res.ok && res.data) {
    const d = res.data;
    state.totalHours = d.totalHours ?? 0;
    state.daysPresent = d.daysPresent ?? 0;
    state.daysAbsent = d.daysAbsent ?? 0;
    state.attendance = d.attendance ?? 0;
    state.attendanceRecorded = d.attendanceRecorded ?? false;
    state.requirements = d.requirements ?? [];
    state.application = d.application ?? null;
    state.attendanceLogs = d.attendanceLogs ?? [];
    state.tasks = d.tasks ?? [];
    state.journals = d.journals ?? [];
    state.reports = d.reports ?? [];
    state.supervisorData = d.supervisorData ?? { students: [] };
    state.allUsers = d.allUsers ?? [];
    
    // Auto-derive recent activities for student view
    state.activities = [];
    if (state.application) {
      state.activities.push({ icon: "♟", title: "Placement application active", time: state.application.dateSubmitted || "Recent" });
    }
    if (state.attendanceLogs.length > 0) {
      state.activities.push({ icon: "◷", title: `Logged attendance (${state.attendanceLogs[0].date})`, time: "Recent" });
    }
    if (state.journals.length > 0) {
      state.activities.push({ icon: "✎", title: `Submitted journal: ${state.journals[0].title}`, time: state.journals[0].date });
    }
    if (state.tasks.length > 0) {
      state.activities.push({ icon: "≡", title: `Task: ${state.tasks[0].title}`, time: state.tasks[0].due || "Assigned" });
    }
  }
}
  ...
  const data = {
    ...
    admin: {
      application: [
        ...
        `${state.allUsers.length} active accounts`,
        "Add user",
        "user",
      ],
      requirements: [
        ...
        "4 requirement types are active",
        "Add requirement",
        "requirement",
      ],
      attendance: [
        ...
        "Active practicum period scheduled",
        "Manage period",
        "period",
      ],
      feedback: [
...
function renderSupervisorView(view) {
  ...
  switch (view) {
    case "application":
      return `<div class="page">${pageIntro("Department Trainees", "Assigned OJT students", "View placement agreements and company details for students in your department.", '<button class="primary-button" data-modal="placement">View placement <span>→</span></button>')}<section class="hero-strip"><div><h2>${count} student${count === 1 ? "" : "s"} assigned to your department</h2><p>Review placement details and mentor assignments.</p></div><div class="hero-stat"><strong>${count < 10 ? "0" + count : count}</strong><span>assigned</span></div></section><section class="panel"><div class="panel-header"><div><h3>Assigned students & placements</h3><p>Active trainees in your department</p></div><button class="text-button" id="refreshBtn">Refresh</button></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Student name</th><th>Program & Department</th><th>Host Organization</th><th>Rendered Hours</th><th>Status</th><th>Action</th></tr></thead><tbody>${
        count === 0
...
function renderOverview() {
  ...
  const totalReqs = (state.requirements || []).length || 4;
...
function renderRoleOverview(role) {
  ...
  const roleData = {
    ...
    admin: {
      ...
      value: `${(state.allUsers || []).length < 10 ? "0" + (state.allUsers || []).length : (state.allUsers || []).length}`,
    ...
    role === "admin"
      ? (state.allUsers || []).length === 0
        ...
        : state.allUsers
            .map(
              (acc) =>
                `<tr><td><strong>${escapeHtml(acc.name)}</strong><br><small style="color:var(--muted)">@${escapeHtml(acc.username)}</small></td><td>${escapeHtml(acc.role)} · ${escapeHtml(acc.department)}</td><td><span class="status status-green">Active</span></td></tr>`,
            )
            .join("")
...
function renderApplication() {
  ...
  return `<div class="page">${pageIntro("Student workspace", "Practicum application", "Manage your host organization placement details and application status.", '<button class="primary-button" data-modal="application">Update application <span>→</span></button>')}<div class="detail-grid"><section class="panel"><div class="panel-header"><div><h3>Application status</h3><p>${app ? "Active placement application" : "Not yet submitted"}</p></div><span class="status ${app ? "status-green" : "status-yellow"}">${app ? "Approved" : "Pending Submission"}</span></div><dl class="info-list"><div class="info-row"><dt>Application ID</dt><dd>${app ? escapeHtml(app.id) : "—"}</dd></div><div class="info-row"><dt>Date submitted</dt><dd>${app ? escapeHtml(app.dateSubmitted) : "—"}</dd></div><div class="info-row"><dt>Placement period</dt><dd>${app ? escapeHtml(app.period) : "—"}</dd></div></dl></section><section class="panel"><div class="panel-header"><div><h3>Host organization</h3><p>Your OJT placement</p></div><span class="status ${app ? "status-blue" : "status-yellow"}">${app ? "Active" : "Unassigned"}</span></div><dl class="info-list"><div class="info-row"><dt>Company</dt><dd>${app ? escapeHtml(app.company) : "Unassigned"}</dd></div><div class="info-row"><dt>Department</dt><dd>${app ? escapeHtml(app.department) : "Unassigned"}</dd></div><div class="info-row"><dt>Supervisor</dt><dd>${app ? escapeHtml(app.supervisor) : "Unassigned"}</dd></div><div class="info-row"><dt>Office hours</dt><dd>${app ? escapeHtml(app.officeHours) : "—"}</dd></div></dl></section></div><section class="panel" style="margin-top:16px"><div class="panel-header"><div><h3>Application timeline</h3><p>Milestones from submission to placement</p></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Milestone</th><th>Date</th><th>Status</th></tr></thead><tbody>${
    app
      ? `<tr><td>Application submitted</td><td>${escapeHtml(app.dateSubmitted)}</td><td><span class="status status-green">Completed</span></td></tr>`
...
function renderRequirements() {
  ...
  return `<div class="page">${pageIntro("Document center", "Your requirements", "Submit and monitor the documents needed for practicum completion.", '<button class="primary-button" data-modal="requirement">Upload document <span>↑</span></button>')}<section class="panel"><div class="panel-header"><div><h3>Submission checklist</h3><p>${approvedCount} of ${reqs.length} requirements completed</p></div><span class="status ${approvedCount === reqs.length && reqs.length > 0 ? "status-green" : "status-yellow"}">${reqs.length - approvedCount} pending</span></div><div class="task-list">${reqs
    .map(
      (r) =>
  ...
  return `<div class="page">${pageIntro("Your workload", "Assigned tasks", "Stay on top of deliverables and tasks assigned by your supervisor.", '<button class="primary-button" data-modal="task">Add personal task <span>+</span></button>')}<section class="panel"><div class="panel-header"><div><h3>Active task list</h3><p>${(state.tasks || []).length} task${(state.tasks || []).length === 1 ? "" : "s"} tracked</p></div></div><div class="task-list">${
    (state.tasks || []).length === 0
      ...
      : state.tasks
          .map(
            (task) =>
              `<label class="task-row"><input class="task-check" data-task-id="${task.id}" type="checkbox" ${task.done ? "checked" : ""}><div><strong>${escapeHtml(task.title)}</strong><small>${escapeHtml(task.meta || "Task")}</small></div><span class="task-due">${escapeHtml(task.due || "—")}</span></label>`,
          )
          .join("")
  }</div></section></div>`;
}
  ...
  return `<div class="page">${pageIntro("Daily reflection", "OJT journal", "Document the tasks, lessons, and skills learned during your placement.", '<button class="primary-button" data-modal="journal">New journal entry <span>+</span></button>')}<section class="panel"><div class="panel-header"><div><h3>Journal entries</h3><p>Keep a consistent record of your practicum experience.</p></div></div><div class="task-list">${
    (state.journals || []).length === 0
      ...
      : state.journals
          .map(
            (j) =>
              `<div class="task-row"><span class="activity-dot">✎</span><div><strong>${escapeHtml(j.title)}</strong><small>${escapeHtml(j.date)} · ${escapeHtml(j.hours || "8h")}</small><p style="margin:6px 0 0;font-size:12px;color:var(--muted)">${escapeHtml(j.reflection)}</p></div><span class="status ${j.status === "Approved" ? "status-green" : j.status === "Revision" ? "status-coral" : "status-yellow"}">${escapeHtml(j.status)}</span></div>`,
          )
          .join("")
  }</div></section></div>`;
}
...
function bindViewActions() {
  ...
  if (refreshBtn) {
    refreshBtn.addEventListener("click", async () => {
      ...
      renderView(activeNav ? activeNav.dataset.view : "overview");
  ...
  if (clock)
    clock.addEventListener("click", async () => {
      ...
      if (res.ok) {
        showToast("Today's attendance recorded successfully.");
        await refreshBackendState();
        renderView("attendance");
      } else {
        showToast(res.data?.message || "Failed to record attendance.");
      }
    });
  ...
  document.querySelectorAll("[data-task-id]").forEach((input) =>
    input.addEventListener("change", async () => {
      ...
      await apiRequest(`/practicum/task/${taskId}/toggle`, "POST", { done: input.checked });
      showToast(
        input.checked
...
function bindPassiveControls() {
  document.querySelectorAll(".task-check:not([data-task-id])").forEach((input) => {
    input.disabled = true;
...
document.getElementById("modalForm").addEventListener("submit", async (e) => {
  ...
  if (action === "application") endpoint = "/practicum/application";
  else if (action === "task" || action === "assign") endpoint = "/practicum/task";
  else if (action === "journal") endpoint = "/practicum/journal";
  else if (action === "report") endpoint = "/practicum/report";
  else if (action === "requirement") endpoint = "/practicum/requirement";
  else if (action === "evaluation") endpoint = "/practicum/evaluation";
  else if (action === "feedback") endpoint = "/practicum/feedback";
  else if (action === "attendance_review") endpoint = "/practicum/review-attendance";
  else if (action === "journal_review") endpoint = "/practicum/review-journal";
  else if (action === "placement") endpoint = "/practicum/update-placement";
  else if (action === "user") endpoint = "/practicum/admin/user";
  ...
  if (endpoint) {
    ...
    if (res.ok) {
      ...
      showToast(res.data?.message || res.data?.error || "Error saving changes.");
    }
  } else {
    closeModal();
...
document.getElementById("loginForm").addEventListener("submit", async (event) => {
  ...
  if (res.ok && res.data?.user) {
    ...
    showToast(res.data.message || `Welcome back, ${res.data.user.name}!`);
    await showApp(res.data.user);
  } else {
    errorElem.textContent = res.data?.message || "Incorrect username or password. Please check your credentials.";
  }
});
...
document.getElementById("signUpForm").addEventListener("submit", async (event) => {
  ...
  if (res.ok && res.data?.user) {
    errorElem.textContent = "";
    showToast(res.data.message || `Account created successfully! Welcome, ${name}.`);
    await showApp(res.data.user);
  } else {
    errorElem.textContent = res.data?.message || "Registration failed. Please check your input.";
  }
});
...
// Auto Check Authenticated Session on Page Load
